...
```bash
docker run -p 2181:2181 -p 9092:9092 --env ADVERTISED_HOST=`docker-machine ip \`docker-machine active\`` --env ADVERTISED_PORT=9092 blademainer/kafka
```
...
Take the same parameters as the blademainer/kafka image with some new ones:
...
https://registry.hub.docker.com/u/blademainer/kafka/

https://registry.hub.docker.com/u/blademainer/kafkaproxy/

Build from Source
---

    docker build -t blademainer/kafka kafka/
    docker build -t blademainer/kafkaproxy kafkaproxy/
